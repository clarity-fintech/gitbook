...
tier: dev_portal
module: "livePayConfig.js"
status_checker: "checkLivePayConfigSwarm"
---

# Layer 028 — livePayConfig LIVE_PAY gate

<!-- data-required-tier="dev_portal" data-section-id="layer-028" -->

<div data-required-tier="dev_portal" data-section-id="layer-028">

## Overview

Architecture layer **28** of 100 · Phase **3** (Execution Billing).

livePayConfig LIVE_PAY gate — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **livePayConfig.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkLivePayConfigSwarm`.
...
```bash
clrty layer:028:health    # probe layer 28 on chain 1202
clrty layer:028:start     # start layer daemon (PM2)
clrty layer:028:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 28
bash scripts/deploy/clrty-node-cli.sh layer start 28
```

## API interface

```yaml
# Layer 028 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 028 — livePayConfig LIVE_PAY gate
  version: "1.0.0"
paths:
  /v1/swarm/layer/028/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 028 healthy on chain 1202
  /v1/swarm/layer/028/status:
    get:
      summary: Module livePayConfig.js status
  /v1/auth/verify:
    post:
