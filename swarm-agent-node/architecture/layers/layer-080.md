...
status_checker: "checkNoDockerEnforcement"
---

# Layer 080 — NO Docker enforcement flag

<!-- data-required-tier="dev_portal" data-section-id="layer-080" -->

<div data-required-tier="dev_portal" data-section-id="layer-080">

## Overview

Architecture layer **80** of 100 · Phase **8** (Cherry Bare-Metal).

NO Docker enforcement flag — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkNoDockerEnforcement`.
...
```bash
clrty layer:080:health    # probe layer 80 on chain 1202
clrty layer:080:start     # start layer daemon (PM2)
clrty layer:080:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 80
bash scripts/deploy/clrty-node-cli.sh layer start 80
```

## API interface

```yaml
# Layer 080 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 080 — NO Docker enforcement flag
  version: "1.0.0"
paths:
  /v1/swarm/layer/080/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 080 healthy on chain 1202
  /v1/swarm/layer/080/status:
    get:
