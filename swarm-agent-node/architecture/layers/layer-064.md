...
tier: dev_portal
module: "ecosystemRoutingManifest.js"
status_checker: "checkEcosystemRoutingSwarm"
---

# Layer 064 — ecosystemRoutingManifest swarm surface

<!-- data-required-tier="dev_portal" data-section-id="layer-064" -->

<div data-required-tier="dev_portal" data-section-id="layer-064">

## Overview

Architecture layer **64** of 100 · Phase **7** (API Gateway Routes).

ecosystemRoutingManifest swarm surface — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **ecosystemRoutingManifest.js** on clrty-1 / chain **1202**.
...
- Verify via status checker `checkEcosystemRoutingSwarm`.
...
```bash
clrty layer:064:health    # probe layer 64 on chain 1202
clrty layer:064:start     # start layer daemon (PM2)
clrty layer:064:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 64
bash scripts/deploy/clrty-node-cli.sh layer start 64
```

## API interface

```yaml
# Layer 064 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 064 — ecosystemRoutingManifest swarm surface
  version: "1.0.0"
paths:
  /v1/swarm/layer/064/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 064 healthy on chain 1202
  /v1/swarm/layer/064/status:
    get:
      summary: Module ecosystemRoutingManifest.js status
  /v1/auth/verify:
    post:
