...
status_checker: "checkPlinkWalletFund"
---

# Layer 015 — plink_clrty_pay_wallet_fund

<!-- data-required-tier="wallet_fund" data-section-id="layer-015" -->

<div data-required-tier="wallet_fund" data-section-id="layer-015">

## Overview

Architecture layer **15** of 100 · Phase **2** (Checkout Offers).

plink_clrty_pay_wallet_fund — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkPlinkWalletFund`.
...
```bash
clrty layer:015:health    # probe layer 15 on chain 1202
clrty layer:015:start     # start layer daemon (PM2)
clrty layer:015:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 15
bash scripts/deploy/clrty-node-cli.sh layer start 15
```

## API interface

```yaml
# Layer 015 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 015 — plink_clrty_pay_wallet_fund
  version: "1.0.0"
paths:
  /v1/swarm/layer/015/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 015 healthy on chain 1202
  /v1/swarm/layer/015/status:
    get:
