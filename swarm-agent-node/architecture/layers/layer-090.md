...
tier: dev_portal
module: "scripts/verify_uiquio_swarms.sh"
status_checker: "checkVerifyUiquioSwarms"
---

# Layer 090 — verify_uiquio_swarms.sh integration

<!-- data-required-tier="dev_portal" data-section-id="layer-090" -->

<div data-required-tier="dev_portal" data-section-id="layer-090">

## Overview

Architecture layer **90** of 100 · Phase **9** (Playwright E2E).

verify_uiquio_swarms.sh integration — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **scripts/verify_uiquio_swarms.sh** on clrty-1 / chain **1202**.
...
- Verify via status checker `checkVerifyUiquioSwarms`.
...
```bash
clrty layer:090:health    # probe layer 90 on chain 1202
clrty layer:090:start     # start layer daemon (PM2)
clrty layer:090:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 90
bash scripts/deploy/clrty-node-cli.sh layer start 90
```

## API interface

```yaml
# Layer 090 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 090 — verify_uiquio_swarms.sh integration
  version: "1.0.0"
paths:
  /v1/swarm/layer/090/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 090 healthy on chain 1202
  /v1/swarm/layer/090/status:
    get:
      summary: Module scripts_verify_uiquio_swarms.sh status
  /v1/auth/verify:
    post:
