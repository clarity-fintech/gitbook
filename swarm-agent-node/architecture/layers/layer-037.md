...
status_checker: "checkResolveSwarmDebitCents"
---

# Layer 037 — resolveSwarmDebitCents helper

<!-- data-required-tier="dev_portal" data-section-id="layer-037" -->

<div data-required-tier="dev_portal" data-section-id="layer-037">

## Overview

Architecture layer **37** of 100 · Phase **4** (Per-Run Agent SKUs).

resolveSwarmDebitCents helper — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkResolveSwarmDebitCents`.
...
```bash
clrty layer:037:health    # probe layer 37 on chain 1202
clrty layer:037:start     # start layer daemon (PM2)
clrty layer:037:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 37
bash scripts/deploy/clrty-node-cli.sh layer start 37
```

## API interface

```yaml
# Layer 037 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 037 — resolveSwarmDebitCents helper
  version: "1.0.0"
paths:
  /v1/swarm/layer/037/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 037 healthy on chain 1202
  /v1/swarm/layer/037/status:
    get:
