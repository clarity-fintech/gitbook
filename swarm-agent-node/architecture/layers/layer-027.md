...
tier: dev_portal
module: "fees.js"
status_checker: "checkExternalAppendFees"
---

# Layer 027 — external_append fee mode

<!-- data-required-tier="dev_portal" data-section-id="layer-027" -->

<div data-required-tier="dev_portal" data-section-id="layer-027">

## Overview

Architecture layer **27** of 100 · Phase **3** (Execution Billing).

external_append fee mode — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkExternalAppendFees`.
...
```bash
clrty layer:027:health    # probe layer 27 on chain 1202
clrty layer:027:start     # start layer daemon (PM2)
clrty layer:027:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 27
bash scripts/deploy/clrty-node-cli.sh layer start 27
```

## API interface

```yaml
# Layer 027 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 027 — external_append fee mode
  version: "1.0.0"
paths:
  /v1/swarm/layer/027/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 027 healthy on chain 1202
  /v1/swarm/layer/027/status:
    get:
