...
status_checker: "checkSwarmNodeInstallPath"
---

# Layer 072 — /opt/clrty/swarm-node install path

<!-- data-required-tier="dev_portal" data-section-id="layer-072" -->

<div data-required-tier="dev_portal" data-section-id="layer-072">

## Overview

Architecture layer **72** of 100 · Phase **8** (Cherry Bare-Metal).

/opt/clrty/swarm-node install path — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkSwarmNodeInstallPath`.
...
```bash
clrty layer:072:health    # probe layer 72 on chain 1202
clrty layer:072:start     # start layer daemon (PM2)
clrty layer:072:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 72
bash scripts/deploy/clrty-node-cli.sh layer start 72
```

## API interface

```yaml
# Layer 072 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 072 — /opt/clrty/swarm-node install path
  version: "1.0.0"
paths:
  /v1/swarm/layer/072/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 072 healthy on chain 1202
  /v1/swarm/layer/072/status:
    get:
