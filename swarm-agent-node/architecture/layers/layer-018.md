...
tier: starter
module: "productionPaymentRails.js"
status_checker: "checkProductionPaymentRailsSwarm"
---

# Layer 018 — productionPaymentRails integration

<!-- data-required-tier="starter" data-section-id="layer-018" -->

<div data-required-tier="starter" data-section-id="layer-018">

## Overview

Architecture layer **18** of 100 · Phase **2** (Checkout Offers).

productionPaymentRails integration — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **productionPaymentRails.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkProductionPaymentRailsSwarm`.
...
```bash
clrty layer:018:health    # probe layer 18 on chain 1202
clrty layer:018:start     # start layer daemon (PM2)
clrty layer:018:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 18
bash scripts/deploy/clrty-node-cli.sh layer start 18
```

## API interface

```yaml
# Layer 018 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 018 — productionPaymentRails integration
  version: "1.0.0"
paths:
  /v1/swarm/layer/018/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 018 healthy on chain 1202
  /v1/swarm/layer/018/status:
    get:
      summary: Module productionPaymentRails.js status
  /v1/auth/verify:
    post:
