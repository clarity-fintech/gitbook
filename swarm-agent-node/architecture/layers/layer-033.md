...
status_checker: "checkAgentClrtySkills005"
---

# Layer 033 — agent_clrty_skills_005 $0.05/run

<!-- data-required-tier="dev_portal" data-section-id="layer-033" -->

<div data-required-tier="dev_portal" data-section-id="layer-033">

## Overview

Architecture layer **33** of 100 · Phase **4** (Per-Run Agent SKUs).

agent_clrty_skills_005 $0.05/run — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkAgentClrtySkills005`.
...
```bash
clrty layer:033:health    # probe layer 33 on chain 1202
clrty layer:033:start     # start layer daemon (PM2)
clrty layer:033:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 33
bash scripts/deploy/clrty-node-cli.sh layer start 33
```

## API interface

```yaml
# Layer 033 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 033 — agent_clrty_skills_005 $0.05/run
  version: "1.0.0"
paths:
  /v1/swarm/layer/033/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 033 healthy on chain 1202
  /v1/swarm/layer/033/status:
    get:
