...
tier: dev_portal
module: "ecosystemInterlockEngine.js"
status_checker: "checkEcosystemInterlockSwarm"
---

# Layer 096 — ecosystemInterlockEngine hooks

<!-- data-required-tier="dev_portal" data-section-id="layer-096" -->

<div data-required-tier="dev_portal" data-section-id="layer-096">

## Overview

Architecture layer **96** of 100 · Phase **10** (MIS & Integration).

ecosystemInterlockEngine hooks — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **ecosystemInterlockEngine.js** on clrty-1 / chain **1202**.
...
- Verify via status checker `checkEcosystemInterlockSwarm`.
...
```bash
clrty layer:096:health    # probe layer 96 on chain 1202
clrty layer:096:start     # start layer daemon (PM2)
clrty layer:096:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 96
bash scripts/deploy/clrty-node-cli.sh layer start 96
```

## API interface

```yaml
# Layer 096 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 096 — ecosystemInterlockEngine hooks
  version: "1.0.0"
paths:
  /v1/swarm/layer/096/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 096 healthy on chain 1202
  /v1/swarm/layer/096/status:
    get:
      summary: Module ecosystemInterlockEngine.js status
  /v1/auth/verify:
    post:
