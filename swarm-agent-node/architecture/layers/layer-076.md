...
tier: dev_portal
module: "systemd/clrty-swarm-node.service"
status_checker: "checkSystemdService"
---

# Layer 076 — systemd clrty-swarm-node.service

<!-- data-required-tier="dev_portal" data-section-id="layer-076" -->

<div data-required-tier="dev_portal" data-section-id="layer-076">

## Overview

Architecture layer **76** of 100 · Phase **8** (Cherry Bare-Metal).

systemd clrty-swarm-node.service — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **systemd/clrty-swarm-node.service** on clrty-1 / chain **1202**.
...
- Verify via status checker `checkSystemdService`.
...
```bash
clrty layer:076:health    # probe layer 76 on chain 1202
clrty layer:076:start     # start layer daemon (PM2)
clrty layer:076:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 76
bash scripts/deploy/clrty-node-cli.sh layer start 76
```

## API interface

```yaml
# Layer 076 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 076 — systemd clrty-swarm-node.service
  version: "1.0.0"
paths:
  /v1/swarm/layer/076/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 076 healthy on chain 1202
  /v1/swarm/layer/076/status:
    get:
      summary: Module systemd_clrty-swarm-node.service status
  /v1/auth/verify:
    post:
