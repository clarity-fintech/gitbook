...
status_checker: "checkAccessCreditsAgentPerRun"
---

# Layer 023 — accessCreditsEngine agent_per_run debit

<!-- data-required-tier="wallet_fund" data-section-id="layer-023" -->

<div data-required-tier="wallet_fund" data-section-id="layer-023">

## Overview

Architecture layer **23** of 100 · Phase **3** (Execution Billing).

accessCreditsEngine agent_per_run debit — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkAccessCreditsAgentPerRun`.
...
```bash
clrty layer:023:health    # probe layer 23 on chain 1202
clrty layer:023:start     # start layer daemon (PM2)
clrty layer:023:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 23
bash scripts/deploy/clrty-node-cli.sh layer start 23
```

## API interface

```yaml
# Layer 023 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 023 — accessCreditsEngine agent_per_run debit
  version: "1.0.0"
paths:
  /v1/swarm/layer/023/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 023 healthy on chain 1202
  /v1/swarm/layer/023/status:
    get:
