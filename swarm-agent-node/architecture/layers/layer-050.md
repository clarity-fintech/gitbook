...
status_checker: "checkBareMetalNoDocker"
---

# Layer 050 — bare_metal deploy_mode (no Docker)

<!-- data-required-tier="dev_portal" data-section-id="layer-050" -->

<div data-required-tier="dev_portal" data-section-id="layer-050">

## Overview

Architecture layer **50** of 100 · Phase **5** (Pay Gate Session).

bare_metal deploy_mode (no Docker) — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkBareMetalNoDocker`.
...
```bash
clrty layer:050:health    # probe layer 50 on chain 1202
clrty layer:050:start     # start layer daemon (PM2)
clrty layer:050:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 50
bash scripts/deploy/clrty-node-cli.sh layer start 50
```

## API interface

```yaml
# Layer 050 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 050 — bare_metal deploy_mode (no Docker)
  version: "1.0.0"
paths:
  /v1/swarm/layer/050/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 050 healthy on chain 1202
  /v1/swarm/layer/050/status:
    get:
