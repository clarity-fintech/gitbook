...
tier: starter
module: "checkoutCatalog.js"
status_checker: "checkCheckoutCatalogSwarmPlinks"
---

# Layer 012 — checkoutCatalog plink index

<!-- data-required-tier="starter" data-section-id="layer-012" -->

<div data-required-tier="starter" data-section-id="layer-012">

## Overview

Architecture layer **12** of 100 · Phase **2** (Checkout Offers).

checkoutCatalog plink index — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **checkoutCatalog.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkCheckoutCatalogSwarmPlinks`.
...
```bash
clrty layer:012:health    # probe layer 12 on chain 1202
clrty layer:012:start     # start layer daemon (PM2)
clrty layer:012:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 12
bash scripts/deploy/clrty-node-cli.sh layer start 12
```

## API interface

```yaml
# Layer 012 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 012 — checkoutCatalog plink index
  version: "1.0.0"
paths:
  /v1/swarm/layer/012/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 012 healthy on chain 1202
  /v1/swarm/layer/012/status:
    get:
      summary: Module checkoutCatalog.js status
  /v1/auth/verify:
    post:
