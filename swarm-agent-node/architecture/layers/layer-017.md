...
status_checker: "checkSwarmCheckoutUrls"
---

# Layer 017 — Spark checkout URL builder

<!-- data-required-tier="starter" data-section-id="layer-017" -->

<div data-required-tier="starter" data-section-id="layer-017">

## Overview

Architecture layer **17** of 100 · Phase **2** (Checkout Offers).

Spark checkout URL builder — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkSwarmCheckoutUrls`.
...
```bash
clrty layer:017:health    # probe layer 17 on chain 1202
clrty layer:017:start     # start layer daemon (PM2)
clrty layer:017:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 17
bash scripts/deploy/clrty-node-cli.sh layer start 17
```

## API interface

```yaml
# Layer 017 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 017 — Spark checkout URL builder
  version: "1.0.0"
paths:
  /v1/swarm/layer/017/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 017 healthy on chain 1202
  /v1/swarm/layer/017/status:
    get:
