...
tier: starter
module: "fees.js"
status_checker: "checkSwarmTreasuryInvariant"
---

# Layer 005 — treasury:clrty-1 invariant

<!-- data-required-tier="starter" data-section-id="layer-005" -->

<div data-required-tier="starter" data-section-id="layer-005">

## Overview

Architecture layer **5** of 100 · Phase **1** (Pricing Tiers SSOT).

treasury:clrty-1 invariant — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **fees.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkSwarmTreasuryInvariant`.
...
```bash
clrty layer:005:health    # probe layer 5 on chain 1202
clrty layer:005:start     # start layer daemon (PM2)
clrty layer:005:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 5
bash scripts/deploy/clrty-node-cli.sh layer start 5
```

## API interface

```yaml
# Layer 005 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 005 — treasury:clrty-1 invariant
  version: "1.0.0"
paths:
  /v1/swarm/layer/005/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 005 healthy on chain 1202
  /v1/swarm/layer/005/status:
    get:
      summary: Module fees.js status
  /v1/auth/verify:
    post:
