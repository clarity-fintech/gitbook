...
status_checker: "checkWalletLoginHandoff"
---

# Layer 046 — wallet login handoff redirect

<!-- data-required-tier="wallet_fund" data-section-id="layer-046" -->

<div data-required-tier="wallet_fund" data-section-id="layer-046">

## Overview

Architecture layer **46** of 100 · Phase **5** (Pay Gate Session).

wallet login handoff redirect — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkWalletLoginHandoff`.
...
```bash
clrty layer:046:health    # probe layer 46 on chain 1202
clrty layer:046:start     # start layer daemon (PM2)
clrty layer:046:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 46
bash scripts/deploy/clrty-node-cli.sh layer start 46
```

## API interface

```yaml
# Layer 046 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 046 — wallet login handoff redirect
  version: "1.0.0"
paths:
  /v1/swarm/layer/046/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 046 healthy on chain 1202
  /v1/swarm/layer/046/status:
    get:
