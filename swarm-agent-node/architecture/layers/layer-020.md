...
tier: wallet_fund
module: "clarityWalletSession.js"
status_checker: "checkClarityWalletHandoff"
---

# Layer 020 — clarityWalletSession handoff

<!-- data-required-tier="wallet_fund" data-section-id="layer-020" -->

<div data-required-tier="wallet_fund" data-section-id="layer-020">

## Overview

Architecture layer **20** of 100 · Phase **2** (Checkout Offers).

clarityWalletSession handoff — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **clarityWalletSession.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkClarityWalletHandoff`.
...
```bash
clrty layer:020:health    # probe layer 20 on chain 1202
clrty layer:020:start     # start layer daemon (PM2)
clrty layer:020:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 20
bash scripts/deploy/clrty-node-cli.sh layer start 20
```

## API interface

```yaml
# Layer 020 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 020 — clarityWalletSession handoff
  version: "1.0.0"
paths:
  /v1/swarm/layer/020/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 020 healthy on chain 1202
  /v1/swarm/layer/020/status:
    get:
      summary: Module clarityWalletSession.js status
  /v1/auth/verify:
    post:
