...
status_checker: "checkPerRunPlinkCatalog"
---

# Layer 036 — Per-run plink catalog entries

<!-- data-required-tier="dev_portal" data-section-id="layer-036" -->

<div data-required-tier="dev_portal" data-section-id="layer-036">

## Overview

Architecture layer **36** of 100 · Phase **4** (Per-Run Agent SKUs).

Per-run plink catalog entries — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkPerRunPlinkCatalog`.
...
```bash
clrty layer:036:health    # probe layer 36 on chain 1202
clrty layer:036:start     # start layer daemon (PM2)
clrty layer:036:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 36
bash scripts/deploy/clrty-node-cli.sh layer start 36
```

## API interface

```yaml
# Layer 036 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 036 — Per-run plink catalog entries
  version: "1.0.0"
paths:
  /v1/swarm/layer/036/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 036 healthy on chain 1202
  /v1/swarm/layer/036/status:
    get:
