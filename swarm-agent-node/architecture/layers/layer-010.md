...
status_checker: "checkUiquioMvpMonetizationCopy"
---

# Layer 010 — UIQUO MVP monetization copy SSOT

<!-- data-required-tier="starter" data-section-id="layer-010" -->

<div data-required-tier="starter" data-section-id="layer-010">

## Overview

Architecture layer **10** of 100 · Phase **1** (Pricing Tiers SSOT).

UIQUO MVP monetization copy SSOT — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkUiquioMvpMonetizationCopy`.
...
```bash
clrty layer:010:health    # probe layer 10 on chain 1202
clrty layer:010:start     # start layer daemon (PM2)
clrty layer:010:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 10
bash scripts/deploy/clrty-node-cli.sh layer start 10
```

## API interface

```yaml
# Layer 010 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 010 — UIQUO MVP monetization copy SSOT
  version: "1.0.0"
paths:
  /v1/swarm/layer/010/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 010 healthy on chain 1202
  /v1/swarm/layer/010/status:
    get:
