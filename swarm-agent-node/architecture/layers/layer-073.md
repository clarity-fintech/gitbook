...
status_checker: "checkPm2Deps"
---

# Layer 073 — PM2 process manager deps

<!-- data-required-tier="dev_portal" data-section-id="layer-073" -->

<div data-required-tier="dev_portal" data-section-id="layer-073">

## Overview

Architecture layer **73** of 100 · Phase **8** (Cherry Bare-Metal).

PM2 process manager deps — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkPm2Deps`.
...
```bash
clrty layer:073:health    # probe layer 73 on chain 1202
clrty layer:073:start     # start layer daemon (PM2)
clrty layer:073:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 73
bash scripts/deploy/clrty-node-cli.sh layer start 73
```

## API interface

```yaml
# Layer 073 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 073 — PM2 process manager deps
  version: "1.0.0"
paths:
  /v1/swarm/layer/073/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 073 healthy on chain 1202
  /v1/swarm/layer/073/status:
    get:
