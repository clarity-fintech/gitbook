...
status_checker: "checkSwarmNanoStepsEndpoint"
---

# Layer 061 — GET /v1/swarm/nano-steps endpoint

<!-- data-required-tier="dev_portal" data-section-id="layer-061" -->

<div data-required-tier="dev_portal" data-section-id="layer-061">

## Overview

Architecture layer **61** of 100 · Phase **7** (API Gateway Routes).

GET /v1/swarm/nano-steps endpoint — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkSwarmNanoStepsEndpoint`.
...
```bash
clrty layer:061:health    # probe layer 61 on chain 1202
clrty layer:061:start     # start layer daemon (PM2)
clrty layer:061:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 61
bash scripts/deploy/clrty-node-cli.sh layer start 61
```

## API interface

```yaml
# Layer 061 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 061 — GET /v1/swarm/nano-steps endpoint
  version: "1.0.0"
paths:
  /v1/swarm/layer/061/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 061 healthy on chain 1202
  /v1/swarm/layer/061/status:
    get:
