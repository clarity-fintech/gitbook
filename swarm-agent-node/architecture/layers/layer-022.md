...
status_checker: "checkSwarmPayGateModule"
---

# Layer 022 — swarmPayGate.js session module

<!-- data-required-tier="dev_portal" data-section-id="layer-022" -->

<div data-required-tier="dev_portal" data-section-id="layer-022">

## Overview

Architecture layer **22** of 100 · Phase **3** (Execution Billing).

swarmPayGate.js session module — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkSwarmPayGateModule`.
...
```bash
clrty layer:022:health    # probe layer 22 on chain 1202
clrty layer:022:start     # start layer daemon (PM2)
clrty layer:022:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 22
bash scripts/deploy/clrty-node-cli.sh layer start 22
```

## API interface

```yaml
# Layer 022 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 022 — swarmPayGate.js session module
  version: "1.0.0"
paths:
  /v1/swarm/layer/022/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 022 healthy on chain 1202
  /v1/swarm/layer/022/status:
    get:
