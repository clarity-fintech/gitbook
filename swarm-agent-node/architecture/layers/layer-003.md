...
status_checker: "checkTier02WebbStarter"
---

# Layer 003 — Tier 02 webb_starter_agents $5.99/mo

<!-- data-required-tier="starter" data-section-id="layer-003" -->

<div data-required-tier="starter" data-section-id="layer-003">

## Overview

Architecture layer **3** of 100 · Phase **1** (Pricing Tiers SSOT).

Tier 02 webb_starter_agents $5.99/mo — settlement on **clrty-1** / chain ID **1202**.
...
- Verify via status checker `checkTier02WebbStarter`.
...
```bash
clrty layer:003:health    # probe layer 3 on chain 1202
clrty layer:003:start     # start layer daemon (PM2)
clrty layer:003:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 3
bash scripts/deploy/clrty-node-cli.sh layer start 3
```

## API interface

```yaml
# Layer 003 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 003 — Tier 02 webb_starter_agents $5.99/mo
  version: "1.0.0"
paths:
  /v1/swarm/layer/003/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 003 healthy on chain 1202
  /v1/swarm/layer/003/status:
    get:
