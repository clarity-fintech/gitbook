...
tier: dev_portal
module: "universalEcosystemPaymentPipeline.js"
status_checker: "checkUniversalEcosystemPipeline"
---

# Layer 026 — universalEcosystemPaymentPipeline

<!-- data-required-tier="dev_portal" data-section-id="layer-026" -->

<div data-required-tier="dev_portal" data-section-id="layer-026">

## Overview

Architecture layer **26** of 100 · Phase **3** (Execution Billing).

universalEcosystemPaymentPipeline — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **universalEcosystemPaymentPipeline.js** on clrty-1 / chain **1202**.
- Honor treasury `treasury:clrty-1`, UUID idempotency, and 3000ms FX TTL invariants.
- Verify via status checker `checkUniversalEcosystemPipeline`.
...
```bash
clrty layer:026:health    # probe layer 26 on chain 1202
clrty layer:026:start     # start layer daemon (PM2)
clrty layer:026:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 26
bash scripts/deploy/clrty-node-cli.sh layer start 26
```

## API interface

```yaml
# Layer 026 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 026 — universalEcosystemPaymentPipeline
  version: "1.0.0"
paths:
  /v1/swarm/layer/026/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 026 healthy on chain 1202
  /v1/swarm/layer/026/status:
    get:
      summary: Module universalEcosystemPaymentPipeline.js status
  /v1/auth/verify:
    post:
