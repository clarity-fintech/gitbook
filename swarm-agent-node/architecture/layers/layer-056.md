...
tier: wallet_fund
module: "shared/components/UsdcTreasuryPayPanel.jsx"
status_checker: "checkUsdcTreasuryPanel"
---

# Layer 056 — UsdcTreasuryPayPanel integration

<!-- data-required-tier="wallet_fund" data-section-id="layer-056" -->

<div data-required-tier="wallet_fund" data-section-id="layer-056">

## Overview

Architecture layer **56** of 100 · Phase **6** (Wallet & Credits).

UsdcTreasuryPayPanel integration — settlement on **clrty-1** / chain ID **1202**.
...
- Maintain SSOT alignment for **shared/components/UsdcTreasuryPayPanel.jsx** on clrty-1 / chain **1202**.
...
- Verify via status checker `checkUsdcTreasuryPanel`.
...
```bash
clrty layer:056:health    # probe layer 56 on chain 1202
clrty layer:056:start     # start layer daemon (PM2)
clrty layer:056:stop      # graceful stop
```

Equivalent via node CLI:

```bash
bash scripts/deploy/clrty-node-cli.sh layer health 56
bash scripts/deploy/clrty-node-cli.sh layer start 56
```

## API interface

```yaml
# Layer 056 API surface (stub)
openapi: 3.0.3
info:
  title: SWARM Layer 056 — UsdcTreasuryPayPanel integration
  version: "1.0.0"
paths:
  /v1/swarm/layer/056/health:
    get:
      summary: Layer health probe
      responses:
        "200":
          description: Layer 056 healthy on chain 1202
  /v1/swarm/layer/056/status:
    get:
      summary: Module shared_components_UsdcTreasuryPayPanel.jsx status
  /v1/auth/verify:
    post:
